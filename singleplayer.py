...
def jeuxs():
    ...
    clock = pg.time.Clock()
    running = True
    counter = 0 #Counnts number of moves a player has taken
    ...
    def scoreboard(stripesIN, solidsIN,player):
        ...
        word = f"Number of balls remaining: {15 - (stripesIN + solidsIN)}"
        textSurf, textRect = text_objects(word, smallText)
        textRect.center = ((w-350), (h-20))
        screen.blit(textSurf, textRect)

        word = f"Number of moves: {counter}"
        textSurf, textRect = text_objects(word, smallText)
    ...
    class Ball:
        ...
        def collide(self, ball): #balls are bouncing back along the line connecting their origins (perpendicular bisector to tangent)
            ...
            try:
                slope = rise/run
                self.angle = math.atan(slope) #angle is the inverse tan of the slope artan only gives angles in quadrant 1 and 3 (CAST rule0)
    ...
    white_ball = WhiteBall(w/2, h/2 + 100, 0)
    balls = [white_ball, Ball(w/2 + 50, h/2 + 40, 1), Ball(w/2 + 90, h/2 - 90, 2), Ball(w/2 - 60, h/2 + 105, 3), Ball(w/2 - 70, h/2 + 85, 4),
             Ball(w/2 - 60, h/2 - 100, 5), Ball(w/2 + 65, h/2 - 70, 6), Ball(w/2 + 50, h/2 - 70, 7),
             Ball(w/2 - 10, h/2 - 30, 8), Ball(w/2 + 45 , h/2 - 30, 9), Ball(w/2 + 50, h/2 - 30, 10),
             Ball(w/2 + 150, h/2 - 40, 11), Ball(w/2 - 200, h/2 - 40, 12), Ball(w/2 + 400, h/2 - 40, 13),
             Ball(w/2 - 90, h/2 - 60, 14), Ball(w/2 + 70, h/2 + 25, 15)] #list of all the balls
    holes = [Hole(w/2, h/2)]
    ...
    def endgame():
        ...
        while end:
            ...
            buttonn('Play again?', w/2-100, h//2+50, 200, 100, (255,153,51), (204,104,0),(255,255,255),jeuxs)

            scoreboardd("Good Job!",w/2,h/3.5)
            scoreboardd("Accuracy: "+ str((15//counter)*100)+"%", w / 2, h / 2)

            if counter > 1:
                scoreboardd(" You finished the game in " + str(counter) +" steps!",w/2,h//2.5)
            else:
                scoreboardd(" You finished the game in " + str(counter) + " step!", w / 2, h // 2.5)
    ...
    while running:
        ...
        allBalls = checker(holes)[0] + checker(holes)[1]

        if allBalls == 14 and blackChecker(holes) == True:
            break
